...
class ImprovedRAGModel:
    ...
    def retrieve_contexts(self, queries: List[str], search_results: List[List[Dict]]) -> List[List[str]]:
        ...
        for query, results in zip(queries, search_results):
            ...
            all_contexts.append([passages[i] for i in top_indices])
        return all_contexts
    ...
    def rerank_contexts(self, query: str, contexts: List[str]) -> List[str]:
        ...
        return ranked_contexts[:5]  # Return top 5 contexts

    def generate_answer(self, query: str, contexts: List[str]) -> str:
        """
        Generate an answer using the selected backend.
        """
        input_text = " ".join([f"Context: {context}" for context in contexts]) + f" Question: {query}"
        if self.use_transformers:
            # Generate with transformers
            return self.generator.generate_response([input_text], max_tokens=50, temperature=0.7, top_p=0.9)[0]
        elif self.is_server:
