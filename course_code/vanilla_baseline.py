...
# Import TransformerModel from transformer_inference.py
...
class InstructModel:
    def __init__(self, llm_name="meta-llama/Llama-3.2-3B-Instruct", is_server=False, vllm_server=None, use_transformers=False):
        """
        Initialize your model(s) here if necessary.
        This is the constructor for your InstructModel class, where you can set up any
        required initialization steps for your model(s) to function correctly.
        """
        self.initialize_models(llm_name, is_server, vllm_server, use_transformers)

    def initialize_models(self, llm_name, is_server, vllm_server, use_transformers):
        ...
        if self.use_transformers:
            # Initialize the TransformerModel from transformer_inference.py
            print("Initializing Transformers model...")
            self.transformer_model = TransformerModel(self.llm_name)
        elif self.is_server:
    ...
    def batch_generate_answer(self, batch: Dict[str, Any]) -> List[str]:
        ...
        if self.use_transformers:
            # Use TransformerModel for generation
            return self.transformer_model.generate_response(prompts=queries, max_new_tokens=50, temperature=0.5, top_p=0.8)
    ...
    def format_prompts(self, queries, query_times):
        ...
        if self.use_transformers:
        # For transformers, no need to format prompts
            return queries
